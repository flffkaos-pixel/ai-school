...
# Short path for the Korean user directory
site_dir = r'C:\Users\������39\site'  # This will be the short path
# Let's find the actual short path
import subprocess
...
DONE_RELS = {
    'content\\resources\\securing_agentic_ai_systems.html',
    'content\\research_updates\\rag_research_table.html',
    'content\\interview_prep\\60_gen_ai_questions.html',
}
...
for root, dirs, fnames in os.walk(content_dir):
    for f in fnames:
        ...
        # Mostly English: Korean letters are small fraction of English
        if eng_letters > 0 and kor_letters < eng_letters / 3:
            files_to_translate.append((filepath, eng_letters, kor_letters, rel))
            print(f'  Will translate: {rel} (E:{eng_letters} K:{kor_letters})')

files_to_translate.sort(key=lambda x: -x[1])
total = len(files_to_translate)
print(f'Found {total} files to retranslate')

for idx, (filepath, eng, kor, rel) in enumerate(files_to_translate):
...
print(f'\nDone! {total} files, {call_count} calls, {total_chars//1000}K chars, {total_time:.1f}m')
